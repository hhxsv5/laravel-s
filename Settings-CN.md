...
- `handle_static`：`bool` 是否开启LaravelS处理静态资源(若Swoole>=1.9.17则由Swoole处理)，默认`false`，建议Nginx处理静态资源，LaravelS仅处理动态资源。静态资源的默认路径为`base_path('public')`，可通过修改`swoole.document_root`变更。

...
| Command | Description |
| --------- | --------- |
| `start` | Start LaravelS, list the processes by "*ps -ef\|grep laravels*". Support the option `-d\|--daemonize` to run as a daemon; Support the option `-e\|--env` to specify the environment to run, such as `--env=testing` will use the configuration file `.env.testing` firstly, this feature requires `Laravel/Lumen 5.2+` |
| `stop` | Stop LaravelS |
